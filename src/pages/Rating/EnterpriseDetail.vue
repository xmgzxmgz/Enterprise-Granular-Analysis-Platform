...
const normalizeRow = (row: Row) => {
  ...
  Object.entries(canonicalMap).forEach(([camel, snake]) => {
    const v = (out as any)[camel]
    const cur = (out as any)[snake]
    if (
      v !== undefined &&
      v !== null &&
    ...
    delete (out as any)[camel]
  })
  ;(out as any).etps_name = (out as any).etps_name || (out as any).name || ''
  return out
}
...
const autoLabelCn = (key: string) => {
  ...
  if (k.includes('contact') || k.includes('phone') || k.includes('tel')) return '联系方式'
