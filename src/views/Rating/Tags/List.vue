...
type TagRow = { name: string; desc?: string; color: 'grey' | 'blue'; source: 'manual' | 'model' | 'mixed'; creator?: string; createdAt?: number; createdAtText?: string }
...
const loadMeta = () => {
  try { return JSON.parse(localStorage.getItem('egap_tag_meta') || '{}') || {} } catch { return {} }
}
const saveMeta = (meta: Record<string, any>) => {
  try { localStorage.setItem('egap_tag_meta', JSON.stringify(meta)) } catch {}
}

onMounted(async () => {
  let base: TagRow[] = []
  try { const data = await getTags(); base = (data?.rows || []).map((r: any) => ({ name: r.name, desc: r.desc, color: 'blue', source: 'model' })) } catch { error.value = true }
  try {
    ...
    for (const r of info?.rows || []) {
      ...
      for (const t of manuals) {
        ...
        tagMap[t] = { name: t, color: 'grey', source: tagMap[t]?.source === 'model' ? 'mixed' : 'manual', creator, createdAt, createdAtText: formatTime(createdAt) }
        meta[t] = { creator, createdAt }
      }
      for (const t of models) {
        const prev = tagMap[t]
        tagMap[t] = { name: t, color: 'blue', source: prev ? 'mixed' : 'model', createdAt: prev?.createdAt, creator: prev?.creator, createdAtText: prev?.createdAtText }
      }
    }
    saveMeta(meta)
    ...
    for (const b of base) {
      ...
      if (!ex) combined.push(b)
        ...
        if (ex.source === 'model' && ex.color !== 'blue') { ex.color = 'blue' }
      }
    }
    rows.value = combined
  } catch {}
...
watch(keyword, async (k) => {
  const q = k.trim()
  if (!q) return
  try { const data = await getTags(q); const list = (data?.rows || []).map((r: any) => ({ name: r.name, desc: r.desc, color: 'blue', source: 'model' as const })); rows.value = mergeTags(rows.value, list) } catch { error.value = true }
})
...
const onRowClick = (row: TagRow) => {
  router.push({ path: '/rating-profile/标签管理/标签企业列表', query: { tag: row.name } })
}
